...
void updateDisplay() {
    ...
    if (!redModeEnabled) {
      ...
      Paint_DrawRectangle(0, 0, 250, 30, WHITE, DOT_PIXEL_1X1, DRAW_FILL_FULL);
      Paint_DrawString_EN(70,  5,  text1, &Font24, WHITE, RED); //24
...
void setup() {
    ...
    // --- OTA через веб-страницу ---
    server.on("/fw", HTTP_GET, [](){
      server.send(200, "text/html",
        "<form method='POST' action='/fw' enctype='multipart/form-data'>"
        "<input type='file' name='firmware'><input type='submit' value='OTA Update'>"
        "</form>");
    });
    server.on("/fw", HTTP_POST, [](){
        server.send(200, "text/plain", "Done, wait...");
        ESP.restart();
      }, [](){
